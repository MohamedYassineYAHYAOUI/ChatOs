...
public class ClientTest {
	...
	public static final Logger logger = Logger.getLogger(ClientTest.class.getName());
...
//		}
	...
	public static void connectToServer(SocketAddress server, String login) throws IOException {
		try (SocketChannel sc = SocketChannel.open(server)) {
			var bb = ByteBuffer.allocate((2 * Integer.BYTES) + (login.length() * Character.BYTES));
			
			bb.putInt(0);
			bb.putInt(login.length());
			bb.put(UTF8_CHARSET.encode(login));
			sc.write(bb.flip());
			// sc.shutdownOutput();
			ByteBuffer recieveBuff = ByteBuffer.allocate(Integer.BYTES);
			if (!readFully(sc, recieveBuff)) {
				throw new IllegalStateException("read fully false");
			}
			recieveBuff.flip();
			var code = recieveBuff.getInt();
			switch (code) {
			case 10: {
	...
	public static void main(String[] args) throws IOException {
		...
		connectToServer(new InetSocketAddress(args[0], Integer.parseInt(args[1])), args[2]);
